...
docker exec {container_name} bash -c "echo '{b64_cmd}' | base64 -d > /tmp/sparkrun_serve.sh && nohup bash /tmp/sparkrun_serve.sh > /tmp/sparkrun_serve.log 2>&1 & echo \$! > /tmp/sparkrun_serve.pid"
